...
class TestUuidMixinSchema:
    ...
    def test_uuid_mixin_schema_with_invalid_type(self):
        ...
        # Act & Assert
        with pytest.raises(ValidationError) as exc_info:
            UuidMixinSchema(id={"invalid": "dict"})  # Use um dict em vez de número
...
class TestTimestampMixinSchema:
    ...
    def test_timestamp_mixin_schema_with_invalid_type(self):
        ...
        # Act & Assert
        with pytest.raises(ValidationError) as exc_info:
            TimestampMixinSchema(created_at=object())  # Use um objeto genérico

        # Ajuste a mensagem esperada baseada na mensagem real do Pydantic
        assert "Input should be a valid datetime" in str(exc_info.value) \
        or "datetime input should be" in str(exc_info.value)
